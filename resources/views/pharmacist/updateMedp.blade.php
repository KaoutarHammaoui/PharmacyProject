...
<body class="bg-gray-100 font-sans">
    ...
    @include('pharmacist.sidebarp')
        ...
        <section class="flex-1 p-10">
            <div class="max-w-xl mx-auto bg-white shadow-xl rounded-2xl p-8">
                ...
                <form action="{{ route('updateMedp', $product->id) }}" method="POST" class="space-y-5">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        <label class="block text-gray-700 font-medium mb-1" for="equivalents">Equivalences (maintenez Ctrl/Cmd pour sélectionner plusieurs)</label>
                        <select
    name="equivalents[]"
    id="equivalents"
    multiple
    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
>
    {{-- $allProducts comes from editMed method --}}
    {{-- $currentEquivalentIds also comes from editMed method --}}
    @foreach($allProducts as $product_option)
        @if($product_option->id !== $product->id) {{-- Don't list itself as an equivalent --}}
            <option value="{{ $product_option->id }}"
                    {{-- This 'in_array' check pre-selects the current equivalents --}}
                    {{ in_array($product_option->id, old('equivalents', $currentEquivalentIds ?? [])) ? 'selected' : '' }}>
                {{ $product_option->name }}
            </option>
        @endif
    @endforeach
